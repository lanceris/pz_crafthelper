...
CHC_recipes_panel = ISPanel:derive('CHC_recipes_panel')
...
function CHC_recipes_panel:initialise()
    ISPanel.initialise(self)
end

function CHC_recipes_panel:createChildren()
    ...
    local timeX = 16 + utils.strWidth(mainSecFont, timeText)
    ...
    self.craftOneButton:setWidth(10 + utils.strWidth(UIFont.Small, self.craftOneButton.title))
    ...
    self.addRandomButton:setWidth(10 + utils.strWidth(UIFont.Small, self.addRandomButton.title))
    ...
    self.selectSpecificButton:setWidth(10 + utils.strWidth(UIFont.Small, self.selectSpecificButton.title))
    ...
    self._buttonObjs = { self.craftOneButton, self.craftAllButton, self.addRandomButton, self.selectSpecificButton }
    -- endregion
    ...
    self.ingredientPanel._internal = "ingredientPanel"
    self.ingredientPanel:initialise()
    self.ingredientPanel:instantiate()
    self.ingredientPanel.rowAt = self.rowAt
    self.ingredientPanel.onMouseWheel = self.onMouseWheel22
    self.ingredientPanel.onRightMouseDown = self.onRMBDownIngrPanel
    self.ingredientPanel.onMouseDown = self.onMouseDown
    self.ingredientPanel.prerender = CHC_view._list.prerender
    self.ingredientPanel._panel = self
    self.ingredientPanel.render = self.renderListBox
    ...
    self.skillPanel._internal = "skillPanel"
    self.skillPanel:initialise()
    self.skillPanel:instantiate()
    self.skillPanel.onMouseWheel = self.onMouseWheel22
    ...
    self.skillPanel.prerender = CHC_view._list.prerender
    self.skillPanel.render = self.renderListBox
    ...
    self.booksPanel._internal = "booksPanel"
    self.booksPanel:initialise()
    self.booksPanel:instantiate()
    self.booksPanel.onMouseWheel = self.onMouseWheel22
    ...
    self.booksPanel.prerender = CHC_view._list.prerender
    self.booksPanel.render = self.renderListBox
    ...
    self.equipmentPanel._internal = "equipmentPanel"
    self.equipmentPanel:initialise()
    self.equipmentPanel:instantiate()
    self.equipmentPanel.onMouseWheel = self.onMouseWheel22
    ...
    self.equipmentPanel.prerender = CHC_view._list.prerender
    self.equipmentPanel.render = self.renderListBox
...
function CHC_recipes_panel:setItemNameInSource(item, itemInList, isDestroy, uses)
...
function CHC_recipes_panel:getSources(recipe)
    local function getCount(item, sourceObj)
        ...
        result = sourceObj.use / param
        if result < 0 then result = -result end
        if floor(result) == 1 then
            displayCount = 1
        else
            local rev = floor(1 / result)
            if rev > 1 then
                displayCount = '(1/' .. rev .. ')'
            elseif rev < 1 then
    ...
    if recipe.isSynthetic then
        ...
        for i = 1, #sources do
            ...
            itemInList.name = self:setItemNameInSource(item, itemInList, sourceInList.isDestroy,
                sourceInList.uses)
            sourceInList.items[#sourceInList.items + 1] = itemInList
            result[#result + 1] = sourceInList
        end
    elseif recipe.isEvolved then
        ...
        for i = 1, #recipe.recipeData.possibleItems do
            ...
            if item.isSpice then
                sourceSpice[#sourceSpice + 1] = item
            else
                sourceMain[#sourceMain + 1] = item
            end
        end
...
function CHC_recipes_panel:setResultObj(resultItem, recipe)
...
function CHC_recipes_panel:setObj(recipe)
...
-- region update
function CHC_recipes_panel:processOneSourceItem(result, item, sourceItemTypes, source, isEvolved)
    local itemFT = item:getFullType()
    local chcItem = CHC_main.items[itemFT]
    if not chcItem then return end
    local isDestroy
    if isEvolved then
        isDestroy = source.isDestroy
    else
        isDestroy = source:isDestroy()
    end
    local uses = isEvolved and source.uses or source:getUse()
    ...
    if sourceItemTypes[itemFT] then
        local count = 1
        if not isDestroy and chcItem.IsDrainable then
            count = item:getDrainableUsesInt()
        end
        if not isDestroy and chcItem.category == "Food" then
            if uses > 0 then
...
function CHC_recipes_panel:getAvailableItemsType()
    local result = {}
    ...
    elseif recipe.isEvolved then
        ...
        -- local _item = CHC_main.common.getConcreteItem(self.containerList, recipe.recipeData.result.fullType)
        -- if not _item then
        --     _item = instanceItem(recipe.recipeData.result.fullType)
        -- end
        ...
        for i = 2, #self.selectedObj.sources do
            ...
            for x = 0, items:size() - 1 do
                ...
                self:processOneSourceItem(result, item, sourceItemTypes, source, true)
            end
        end
    else
        recipe = recipe.recipe
        for i = 0, recipe:getSource():size() - 1 do
            ...
            for x = 0, items:size() - 1 do
                local item = items:get(x)
                self:processOneSourceItem(result, item, sourceItemTypes, source)
            end
        end
    end
    return result
...
function CHC_recipes_panel:shouldUpdateIngredients(selectedItem)
...
function CHC_recipes_panel:refreshIngredientPanel(selectedItem)
    ...
    for i = 1, #selectedItem.sources do
        sortedSources[#sortedSources + 1] = selectedItem.sources[i]
    end
    ...
    for i = 1, #sortedSources do
        ...
        for j = 1, #source.items do
            ...
            if selectedItem.isEvolved and self.evolvedSelected then
                local chcitem = CHC_main.items[item.fullType]
                local isSpice = chcitem.propsMap and
                    chcitem.propsMap["Spice"] or chcitem.isSpice
            ...
            if selectedItem.isEvolved and not evolvedState then
                ...
                unavailable[#unavailable + 1] = data
            elseif selectedItem.typesAvailable and (not numTypes or numTypes < item.count) then
                data.available = false
                unavailable[#unavailable + 1] = data
            else
                data.available = true
                available[#available + 1] = data
            end
        end
        tsort(available, function(a, b) return a.name:lower() < b.name:lower() end)
        tsort(unavailable, function(a, b) return a.name:lower() < b.name:lower() end)
...
function CHC_recipes_panel:refreshSkillPanel(recipe)
    self.skillPanel:clear()
    local w = recipe.requiredSkillCount
    ...
    if w > 3 then w = 3 end
    self.skillPanel:setHeight(w * self.skillPanel.itemheight)
end

function CHC_recipes_panel:refreshBooksPanel(recipe)
    ...
    if numEntries > 3 then numEntries = 3 end
    self.booksPanel:setHeight(numEntries * self.booksPanel.itemheight)
end

function CHC_recipes_panel:refreshEquipmentPanel(recipe)
    ...
    local maxItems = #self.equipmentPanel.items
    if maxItems > 1 then maxItems = 1 end
    self.equipmentPanel:setHeight(maxItems * self.equipmentPanel.itemheight)
end

function CHC_recipes_panel:updateMainInfo(obj)
    ...
    if resultData then
        ...
        self.mainRes:setTooltip(self.mainRes.origTooltip)
        -- self.mainRes:setTooltip(resultData.tooltip)
    ...
    if maxY > 74 then maxY = 74 end
    if maxY < self.mainImg.height then maxY = self.mainImg.height end
    self.mainInfo:setHeight(maxY + 10)
...
function CHC_recipes_panel:updateExtraData(foodData)
    ...
    for i = 1, #foodData do
        ...
        if not item.isCal then
            ...
            btn:initialise()
            btn.width = w
            btn.textW = utils.strWidth(btn.font, btn.title)
    ...
    local maxItems = numBtn
    if maxItems > 8 then maxItems = 8 end
    local mainExtraW = 2 * margin + maxItems * (w + innerMar) - innerMar
...
function CHC_recipes_panel:onResize()
    ...
    local statsH = self.height - self.mainInfo.height - self.padY
...
function CHC_recipes_panel:drawAddToEvolved(y, item, parent, dx)
    local addXPos = 2
    local addW = dx - 2
    local icon
...
function CHC_recipes_panel:drawIngredient(y, item, alt)
...
function CHC_recipes_panel:drawSkill(y, item, alt)
    if not self.recipepanel then
        self.recipepanel = self.parent.parent.parent
    end
...
function CHC_recipes_panel:drawBook(y, item, alt)
    if not self.recipepanel then
        self.recipepanel = self.parent.parent.parent
    end
    local x = 0
...
function CHC_recipes_panel:drawEquipment(y, item, alt)
    if not self.recipepanel then
        self.recipepanel = self.parent.parent.parent
    end
...
function CHC_recipes_panel:renderListBox()
...
-- region temp
-- function CHC_recipes_panel:onIngredientMouseMove(x, y)
...
--         local foodData = CHC_recipes_panel.getFoodData(panel,
...
--         CHC_recipes_panel.updateExtraData(panel, foodData)
...
function CHC_recipes_panel:rowAt(x, y)
    local y0 = 0
    if not self._indexes then return end
    for i = 1, #self._indexes do
        local _ix = self._indexes[i]
        local item = self.items[_ix]
        if not item then return -1 end
        if y >= y0 and y < y0 + item.height then
            return _ix
        end
        y0 = y0 + item.height
    end
...
function CHC_recipes_panel:onMouseWheel22(del)
...
function CHC_recipes_panel:onRMBDownIngrPanel(x, y, item)
...
function CHC_recipes_panel:onRMBDownItemIcon(x, y)
...
function CHC_recipes_panel:onMouseDown(x, y)
    ...
    self.selected = row;
    CHC_recipes_panel.onIngredientMouseDown(self._panel, self.items[self.selected].item)
end

function CHC_recipes_panel:onIngredientMouseDown(item)
    if not item then return end
    ...
    if not item.multipleHeader then
        ...
        if (x >= favXPos) then
            ...
            CHC_menu.playerModData.CHC_item_favorites[item.fullType] = isFav or nil
    ...
    if item.multipleHeader then
        if (x >= favXPos) then
            ...
            if item.blockHiddenState ~= blockHiddenStateSelector[1] or item.collapsed == true then
...
-- region crafting
function CHC_recipes_panel:transferItems()
    local result = {}
    local selectedItem = self.selectedObj;
...
function CHC_recipes_panel:onCraftComplete(completedAction, recipe, container, containers)
    ...
    if not recipe:isCanBeDoneFromFloor() then
        for i = 1, items:size() do
            ...
            if item:getContainer() ~= self.player:getInventory() then
                ...
                returnToContainer[#returnToContainer + 1] = item
            end
        end
    end
...
function CHC_recipes_panel:craft(button, all)
...
function CHC_recipes_panel:craftAll()
    self:craft(nil, true);
end

-- region evolved
function CHC_recipes_panel:addRandomMenu()
    ...
    if used < max and types and not utils.empty(types) then
        tsort(types, function(a, b) return a.name:lower() < b.name:lower() end)
        ...
        if #types > 1 then
            ...
            ingredientSubMenu:addOption(sformat("%s (%s)", getText("IGUI_Evolved_Random"), getText("UI_All")), self,
                CHC_recipes_panel.addRandomCategory, types, true, false)
            ingredientSubMenu:addOption(sformat("%s (%s)", getText("IGUI_Evolved_Random"), getText("ContextMenu_One")),
                self, CHC_recipes_panel.addRandomCategory, types, false, false)
        end
        for i = 1, #types do
            local _type = types[i]
            local optName = getText("ContextMenu_FoodType_" .. _type.name) .. " (" .. #_type.items .. ")"
            local opt = ingredientSubMenu:addOption(optName, self, CHC_recipes_panel.addRandomIngredient, _type.items,
                false, false)
            ...
            if #_type.items == 1 then
                ...
                for j = 1, #types[i].items do
                    ...
                    ingredientCategorySubMenu:addOption(
                        ...
                        self, CHC_recipes_panel.addItemInEvolvedRecipe, item, false, false)
                end
            end
        end
    end
    ...
    if used > 0 and spices and not utils.empty(spices) then
        tsort(spices, function(a, b) return a.item.name:lower() < b.item.name:lower() end)
        ...
        if #spices > 1 then
            condimentSubMenu:addOption(sformat("%s (%s)", getText("IGUI_Evolved_Random"), getText("UI_All")), self,
                CHC_recipes_panel.addRandomCategory, spices, true, true)
            condimentSubMenu:addOption(sformat("%s (%s)", getText("IGUI_Evolved_Random"), getText("ContextMenu_One")),
                self, CHC_recipes_panel.addRandomIngredient, spices, false, true)
        end
        for i = 1, #spices do
            local _spice = spices[i]
            local opt = condimentSubMenu:addOption(
                _spice.item.displayName .. " x " .. _spice.uses, self,
                CHC_recipes_panel.addItemInEvolvedRecipe, _spice, false, true)
        end
    end
    -- endregion
end

function CHC_recipes_panel:addRandomCategory(options, all, isSpice)
...
function CHC_recipes_panel:addRandomIngredient(options, all, isSpice)
    local opt = CHC_main.common.getRandom(options)
    self.addItemInEvolvedRecipe(self, opt, all, isSpice)
end

function CHC_recipes_panel:addItemInEvolvedRecipe(item, all, isSpice)
    local function updateSelected(self)
        self.parent.getContainers(self)
        CHC_recipes_panel.getEvolvedChoices(self, self.selectedObj, self.containerList)
        CHC_recipes_panel.setSpecificEvolvedItem(self, self.evolvedChoices[self.evolvedSelectedIx])
    end
    ...
    local function onComplete(self, completedAction, isSpice)
        ...
        self.evolvedValidTypes, self.evolvedValidSpices = CHC_recipes_panel.getValidEvolvedIngredients(self,
        ...
        item = CHC_main.common.getRandom(options)
        if item.items then
    ...
    if not inv:contains(itemObj) then
        ...
        returnToContainer[#returnToContainer + 1] = itemObj
    end
    if not inv:contains(baseItem) then
        ...
        returnToContainer[#returnToContainer + 1] = baseItem
    end
...
function CHC_recipes_panel:setSpecificEvolvedItem(evolvedOpt)
...
function CHC_recipes_panel:getFoodData(ch)
...
function CHC_recipes_panel:getTooltipForEvolvedItem(ch)
    local function collectOptions(options, contains)
        ...
        for i = 1, #counts do
            ...
            CHC_main.common.cacheTex(value.item)
            local label = "- " .. "<IMAGE:" .. value.item.texture_name .. ",32,32>" .. value.item.displayName
            if value.count > 1 then
                label = label .. " x " .. value.count
            end
            contains[#contains + 1] = label
        end
    end

    local contains = {}
    contains[#contains + 1] = ch.displayNameExtra

    if ch.extraItems then
        contains[#contains + 1] = "<RGB:0.5,0.5,0.5>_______________ <RGB:1,1,1>"
        contains[#contains + 1] = sformat("<RGB:0.8,0.8,0.8> %s <RGB:1,1,1>", getText("Tooltip_item_Contains"))
        collectOptions(ch.extraItems, contains)

        if ch.extraSpices then
            contains[#contains + 1] = sformat("<RGB:0.8,0.8,0.8> %s <RGB:1,1,1>", getText("Tooltip_item_Spices"))
            collectOptions(ch.extraSpices, contains)
        end
    end
    contains[#contains + 1] = ""
    return concat(contains, '\n')
end

function CHC_recipes_panel:selectSpecificMenu()
    local context = ISContextMenu.get(0, getMouseX() + 10, getMouseY())
    ...
    for i = 1, #choices do
        ...
        CHC_main.common.cacheTex(ch.itemObj)
...
function CHC_recipes_panel:getValidEvolvedIngredients(typesAvailable)
    local typesToShow = {}
    local spices = {}
        ...
        if item and item.fullType ~= baseItem then
            ...
            if foodType then
                ...
                if concreteItem and concreteItem.item then
                    ...
                    local _hung = hungChange * 100
                    if _hung < 0 then _hung = -_hung end
                    if use > _hung then
                        use = floor(_hung)
                    end
                    uses = floor(count / use)
                    isValidUses = uses > 0
                ...
                if isValidUses and isValidCooked and not isInvalidFrozen then -- TODO add tooltip
                    if (foodType == "NoExplicit" or isSpice) then
                        if not containsSpices[item.fullType] then
                            spices[#spices + 1] = { item = item, uses = #concreteItems }
                        end
                    else
                ...
                for i = 1, #sourceSpice do
                    if sourceSpice[i].fullType == fullType and not containsSpices[item.fullType] then
                        spices[#spices + 1] = { item = item, uses = 1 }
                    end
                end
            end
        end
    ...
    for foodType, items in pairs(typesToShow) do
        types[#types + 1] = { name = foodType, items = items }
    end
    return types, spices
end

function CHC_recipes_panel:updateButtons(obj)
    self.buttonVisible = false
    local statsY = self.mainInfo.y + self.mainInfo:getBottom() + self.padY
    local statsH = self.height - self.mainInfo:getBottom() + self.padY
    ...
    if obj.available then
        if obj.recipe.isEvolved then
            ...
            if obj.howManyCanCraft > 1 then
                ...
                self.craftAllButton:setWidth(10 + utils.strWidth(UIFont.Small, self.craftAllButton.title))
    ...
    for i = 1, #self._buttonObjs do
        self._buttonObjs[i]:setY(self.mainInfo:getBottom() + 2 * self.padY)
    end
...
function CHC_recipes_panel:handleOverflow(label, maxWidth)
...
function CHC_recipes_panel:getEvolvedChoices(obj, containerList)
...
function CHC_recipes_panel:new(args)
    local o = {};
